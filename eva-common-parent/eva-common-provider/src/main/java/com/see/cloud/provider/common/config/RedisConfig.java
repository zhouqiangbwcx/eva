package com.see.cloud.provider.common.config;

import com.fasterxml.jackson.annotation.JsonAutoDetect;
import com.fasterxml.jackson.annotation.PropertyAccessor;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.see.cloud.core.common.utils.StringUtils;
...
public class RedisConfig extends CachingConfigurerSupport implements EnvironmentAware {
	...
	public JedisPool redisPoolFactory() {
		...
		if (StringUtils.isNoNullOrEmpty(propertyResolver.getProperty("password"))) {// 判断有没密码
			jedisPool = new JedisPool(jedisPoolConfig, propertyResolver.getProperty("host"),
					Integer.valueOf(propertyResolver.getProperty("port")),
					Integer.valueOf(propertyResolver.getProperty("timeout")), propertyResolver.getProperty("password"));
		} else {
			jedisPool = new JedisPool(jedisPoolConfig, propertyResolver.getProperty("host"),
					Integer.valueOf(propertyResolver.getProperty("port")),
	 ...
	 * @Bean
	 * 
	 * @ConditionalOnMissingBean(RedisClient.class)//容器中如果没有RedisClient这个类,
	 * 那么自动配置这个RedisClient public RedisClient
	 * redisClient(@Qualifier("jedisPool")JedisPool pool) { RedisClient
	 * redisClient = new RedisClient(); redisClient.setJedisPool(pool); return
	 * redisClient; }
	 */
}
